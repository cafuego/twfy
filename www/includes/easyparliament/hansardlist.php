...
class HANSARDLIST {
   ...
   *
   */
  public function render($view, $data, $format = 'html', $mobile = 0) {
  ...
  public function _get_data_by_calendar($args) {
    ...
    if ($q->rows() > 0) {
      ...
      for ($row = 0; $row < $q->rows(); $row++) {
        ...
        [$year, $month, $day] = explode('-', $q->field($row, 'hdate'));
  ...
  public function _get_hansard_data($input) {
    ...
    $order        = $input['order'] ?? '';
    $limit        = $input['limit'] ?? '';
    $listurl_args = $input['listurl_args'] ?? [];
      ...
      for ($n = 0; $n < $q->rows(); $n++) {
        ...
        if (isset($this->commentspage)) {
          ...
          if ($this->major == 6) {
            ...
            $COMMENTSURL->insert([$getvar => $item['gid']]);
...
class WRANSLIST extends HANSARDLIST {
  ...
  /**
   * We don't have a separate page for wrans comments.
   */
  public $commentspage = 'wrans';
   ...
   *
   */
  public function _get_data_by_mp($args = []) {
    global $PAGE;
    $data = [];
...
class StandingCommittee extends DEBATELIST {
  ...
  public function _get_committee($bill_id) {
    ...
    for ($i = 0; $i < $q->rows(); $i++) {
      ...
      $mp = new MEMBER(['member_id' => $member_id]);
